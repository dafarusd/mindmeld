...
To run it yourself you need Python 3.10 or later, and Git LFS to fetch the
trained model. Install Git LFS first — `sudo apt install git-lfs` on Debian or
Ubuntu, `brew install git-lfs` on macOS — or `ckpt/brain.fp16.pt` arrives as a
133-byte text stub and the genie falls back to its script without saying why.
...
- Ships as `ckpt/brain.fp16.pt` (53.6MB), stored in Git LFS. Half precision
  halves the file size, and the export is checked to produce byte-identical
  output to the full-precision training checkpoint.
